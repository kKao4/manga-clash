...
export default function AdminImage({ mangaImage, setMangaImage, setFile }: { mangaImage: string, setMangaImage: any, setFile: any }) {
  return (
    <>
      <label
      ...
      >
        <input
          ...
          onChange={(e) => {
            if (e.target.files) {
              setMangaImage(URL.createObjectURL(e.target.files[0]))
              if (e.target.files) {
      ...
      <Image
        className="block w-full h-full mx-auto" src={mangaImage?.startsWith("blob") ? mangaImage : `/${mangaImage}`}
        alt=""
        width="193"
        height="274"
        priority={true}
