...
export default function TextArea({ mangaState, description, setDescription }: { mangaState?: typeof initialMangaState[number], description: string, setDescription: Dispatch<SetStateAction<string>> }) {
  ...
  return (
    <div>
      <div className="pt-1.5 pb-1 px-2 space-x-1 border border-b-transparent border-neutral-400 dark:bg-neutral-700 rounded-t">
        <button
          type="button"
      ...
      <textarea
        name="description"
        className={`w-full px-3 border border-neutral-400 dark:bg-neutral-700 border-t-transparent rounded-b focus:outline-none min-h-[240px] mt-0`}
        value={description}
