    ...
    >
      ...
      <td className={`flex flex-row p-4 pr-0 text-sm gap-x-4`}>
        ...
        <Link
          ...
          className="font-bold transition-colors cursor-pointer line-clamp-5 hover:text-second-green dark:hover:text-third-green"
        >
          {manga.name}
        </Link>
      </td>
          ...
          onClick={async () => {
            ...
            if (bookmarkState.page > Math.ceil((mangasLength - 1) / mangasPerPage)) {
              ...
              if (bookmarkState.page !== 1) {
                ...
                dispatch(setMangasBookmark({ mangas: [], length: 0 }))
              }
            } else {
