...
export default function Navigation({ manga }: { manga: typeof initialMangaState[number] | ChapterResponse["data"] | null }) {
  ...
  return (
    <div className="flex flex-row gap-x-1.5 py-3 pb-2 md:py-4 flex-wrap items-center">
      <Link
        href="/"
        className="text-[13px] md:text-sm transition-colors text-gray-text hover:text-second-green"
      >
        Trang chủ
      </Link>
      ...
      {manga && (
        <>
          ...
          <Link
            ...
            className={`text-[13px] md:text-sm transition-colors ${router.query.mangaHref === manga.href && !("chapter" in manga) ? "text-second-green" : "text-gray-text hover:text-second-green"}`}
          >
            {manga.name}
          </Link>
          {"chapter" in manga && (
