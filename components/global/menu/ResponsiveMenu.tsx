      ...
      <AnimatePresence>
        {showModalMenu && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="w-full h-screen bg-black/50 fixed top-0 left-0 z-20"
          />
        )}
      </AnimatePresence>
      {/* responsive menu */}
      <div ref={myRef} className={`max-w-[274px] h-screen bg-main-green px-8 py-6 transition-all duration-350 fixed z-20 top-0 left-0 md:px-12 md:py-10 ${showModalMenu ? "translate-x-0 opacity-100" : "-translate-x-[300px] opacity-0"}`}>
