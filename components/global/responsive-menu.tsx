...
export default function ResponsiveMenu({
  zIndex, showModalMenu, handleOnClick
}: {
  zIndex: string, showModalMenu: boolean, handleOnClick: any
}) {
  return (
    <>
      ...
      <div className={`w-[300px] h-screen bg-main-green px-8 py-6 transition-transform duration-350 fixed z-20 top-0 left-0 md:px-12 md:py-10 ${showModalMenu ? "translate-x-0" : "-translate-x-[300px]"}`}>
        <div className="flex flex-col">
          ...
          <Link href="/" className="py-6 text-sm text-white uppercase border-b cursor-pointer border-neutral-300">Home</Link>
          <Link href="/manga?page=1&sort=latest" className="py-6 text-sm text-white uppercase border-b cursor-pointer border-neutral-300">Latest Manga</Link>
          <Link href="/manga?page=1&sort=views" className="py-6 text-sm text-white uppercase border-b cursor-pointer border-neutral-300">Hot Manga</Link>
          <Link href="/manga?page=1&sort=new" className="py-6 text-sm text-white uppercase border-b cursor-pointer border-neutral-300">New Manga</Link>
        </div>
      </div>
    </>
  )
